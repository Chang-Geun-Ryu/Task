...
/*:
 ...
 ---
 */
let macBookArray = [macBook, macBook, macBook]

let pEncoder = PropertyListEncoder()
let encodedMacBookArray = try! pEncoder.encode(macBookArray)
print(encodedMacBookArray)


let pDecoder = PropertyListDecoder()
if let decodedMacBookArray = try? pDecoder.decode([MacBook].self, from: encodedMacBookArray) {
  print(decodedMacBookArray)
}

let macBookDict = [0: macBook, 1: macBook, 2: macBook]

let pEncoderDict = PropertyListEncoder()
let encodedMacBookDict = try! pEncoderDict.encode(macBookDict)
print(encodedMacBookDict)

let pDecoderDict = PropertyListDecoder()
if let decodedMacBookDict = try? pDecoderDict.decode([Int: MacBook].self, from: encodedMacBookDict) {
