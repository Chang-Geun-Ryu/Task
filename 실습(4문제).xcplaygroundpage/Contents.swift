...
class Barista {
  ...
  func makeCoffee(menu: CoffeeMenu) -> Coffee {
    print("\(name)에게 ", terminator: "")
    return Coffee(name: menu.rawValue, price: menu.price())
  }
}
...
class Coffee {
  ...
  init(name: String, price: Int) {
    self.name = name
    self.price = price
    print("\(price)원 짜리 \(name)을(를) 주문하였습니다.")
  }
}
...
class Customer {
  ...
  func order(menu: CoffeeMenu, to barista: Barista) -> Coffee {
    print("\(name)이(가) ", terminator: "")
    return barista.makeCoffee(menu: menu)
  }
}
...
print("\n---------- [ 2번 문제 ] ----------\n")
func printDict(array: [Int]) {
  var dict: [Int: [Int]] = [:]
  
  for num in array {
    if let _ = dict[num/10] {
      dict[num/10]?.append(num)
    } else {
      dict.updateValue([num], forKey: num/10)
    }
  }
  for num in 1...10 {
    guard dict[num] != nil else { continue }
    print("Group: \(num) - value: \(dict[num]!)")
  }
}
printDict(array: data1)
printDict(array: data2)
...
func printArray(arrayOrigin: [Int]) {
  var array: [[Int]?] = []
  
  for _ in 1...11 { array.append(nil) }
  for num in arrayOrigin {
    if let _ = array[num/10] {
      array[num/10]?.append(num)
    } else {
      array[num/10] = [num]
    }
  }
  for num in 1...10 {
    guard array[num] != nil else { continue }
    print("Group: \(num) - value: \(array[num]!)")
  }
}

printArray(arrayOrigin: data1)
printArray(arrayOrigin: data2)
...
let odd = array.filter { $0 % 2 != 0 } .map { $0 + 1}
print("Output: ", odd)

let arrayString = ["1", "2", "3", "4", "5", "A", "B", "C", "D"]
let sum = arrayString.map { Int($0) }.compactMap { $0 } .reduce(0) { $0 + Int(pow(Double($1), 2)) }
print(sum)
